...
public class Part2 {
	public static void main(String[] args) throws FileNotFoundException {
		...
		while (text.hasNextLine()) {
			...
			int[] inst = { Integer.parseInt(instrPart1[0].substring(5)), Integer.parseInt(instrPart2[0]) - 1,
					Integer.parseInt(instrPart2[1]) - 1 };
			LinkedList<Character> moved = new LinkedList<Character>();
			for (int i = inst[0]; i > 0; i--) {
				moved.add(crates.get(inst[1]).getLast());
				crates.get(inst[1]).removeLast();
			}
			for (int i = 0; i < inst[0]; i++) {
				crates.get(inst[2]).add(moved.getLast());
				moved.removeLast();
			}
		...
		for (LinkedList<Character> i : crates) {
			for (Character j : i) {
				System.out.print(j);
			}
			System.out.println();
